...
ask_yes_no_default "Do you want to install other packages?" 0 && yay -S btop ferdium-bin chafa ktouch distrobox podman bleachbit kooha k6-bin scc asciiquarium fastfetch cava spotify gitflow-cjs yt-dlp mongodb-compass zoxide glow wl-clipboard mkcert hyprshade jq clapper entr ripgrep yazi eog age obsidian obs-studio lazygit pokemon-colorscripts-git lazydocker-bin just github-cli postman-bin httpie mpv ark jetbrains-toolbox xdg-ninja tmux eza thefuck taskwarrior-tui git-delta fd sd tealdeer bat git-secrets fzf git-interactive-rebase-tool-bin lite-xl gparted vlc ktorrent persepolis

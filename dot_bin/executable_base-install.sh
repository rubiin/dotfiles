...
ask_yes_no_default "Do you want to install other packages?" 0 && yay -S btop ktouch keybase cloc fastfetch gitflow-cjs yt-dlp mongodb-compass zoxide glow wl-clipboard mkcert hyprshade jq clapper entr ripgrep ranger eog age obsidian obs-studio lazygit pokemon-colorscripts-git lazydocker-bin  just github-cli postman-bin httpie  mpv ark jetbrains-toolbox tmux eza thefuck taskwarrior-tui git-delta  grex fd sd tealdeer bat git-secrets fzf git-interactive-rebase-tool-bin mousepad gparted neovim vlc ktorrent persepolis
